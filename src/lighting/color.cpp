...
namespace ST
{
    namespace Lighting
    {
        void Color::createCamera()
        {
            mCamera = new Camera(glm::vec3(0.0f, 0.0f, 10.0f), glm::vec3(0.0f, 1.0f, 0.0f), -90.0f, 0.0f);
        }

        void Color::createShader()
        {
            mLightingShader = new Shader();
            mLightingShader->setVertexShader("./res/shaders/lighting/color.vs");
            mLightingShader->setFragmentShader("./res/shaders/lighting/color.fs");
            mLightingShader->compile();

            mLightCubeShader = new Shader();
            mLightCubeShader->setVertexShader("./res/shaders/lighting/color_light_cube.vs");
            mLightCubeShader->setFragmentShader("./res/shaders/lighting/color_light_cube.fs");
        ...
        void Color::ProcessInput(int a_direction, float a_deltaTime)
        {
            if (mCamera)
            {
        ...
        void Color::ProcessMouseScroll(float a_yOffset)
        {
            if (mCamera)
            {
        ...
        void Color::Update(float a_deltaTime)
        {
        }
