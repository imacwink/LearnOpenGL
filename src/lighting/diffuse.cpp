...
namespace ST
{
    namespace Lighting
    {
        void Diffuse::createCamera()
        {
            mCamera = new Camera(glm::vec3(0.0f, 0.0f, 10.0f), glm::vec3(0.0f, 1.0f, 0.0f), -90.0f, 0.0f);
        }

        void Diffuse::createShader()
        {
            mLightingShader = new Shader();
            mLightingShader->setVertexShader("./res/shaders/lighting/diffuse.vs");
            mLightingShader->setFragmentShader("./res/shaders/lighting/diffuse.fs");
            mLightingShader->compile();

            mLightCubeShader = new Shader();
            mLightCubeShader->setVertexShader("./res/shaders/lighting/diffuse_light_cube.vs");
            mLightCubeShader->setFragmentShader("./res/shaders/lighting/diffuse_light_cube.fs");
        ...
        void Diffuse::ProcessInput(int a_direction, float a_deltaTime)
        {
            if (mCamera)
            {
        ...
        void Diffuse::ProcessMouseScroll(float a_yOffset)
        {
            if (mCamera)
            {
        ...
        void Diffuse::Update(float a_deltaTime)
        {
        }
